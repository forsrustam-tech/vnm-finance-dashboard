...
import { fetchDailyAmoSnapshot, fetchStageEntryCount } from "@/lib/amocrm";
...
export async function syncAmoConnectionForDate(
  ...
  const bookingCount = connection.booking_stage_id
    ? await fetchStageEntryCount(connection.subdomain, connection.access_token, connection.booking_stage_id, fromMs, toMs)
    : 0;

  await sql`
    INSERT INTO amo_daily_snapshots (connection_id, date, new_leads, total_lead_value, won_count, won_revenue, booking_count, by_stage)
    VALUES (${connection.id}, ${dateStr}, ${snap.newLeads}, ${snap.totalLeadValue}, ${snap.wonCount}, ${snap.wonRevenue}, ${bookingCount}, ${JSON.stringify(snap.byStage)})
    ...
    DO UPDATE SET new_leads = ${snap.newLeads}, total_lead_value = ${snap.totalLeadValue},
                   ...
                   booking_count = ${bookingCount}, by_stage = ${JSON.stringify(snap.byStage)}
  `;

  return { ...snap, bookingCount };
}
